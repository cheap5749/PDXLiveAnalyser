
import dictionaries as dico
import fonctions as do
import defines

import time
from datetime import datetime
import csv
import json

# import required module
import os
# assign directory
#directory = 'files'
 
# iterate over files in
# that directory
start_time = time.time()

#obj="population"
#obj="provinces"


    #provs.writerow(["Region","State","param","x","x","x","x","x","x","param","value","x","x","x","x","x","x"])
    #for filename in os.listdir(defines.directory):
        #curfile = os.path.join(defines.directory, filename)
        # checking if it is a file
        #if os.path.isfile(curfile):
           # print(curfile)
    
#import numpy as np

    

#########################################

            
with open(defines.input,'r') as f:
        lines = f.readlines()
        savegame=[]

        for line in lines:
            savegame.append(line)

curdate=savegame[3].replace("\"","").replace("date=","").strip()
year=curdate[:3]
pays=savegame[4].replace("\"","").replace("meta_player_name=","").strip()

cur_level=0
cur_obj=""
prev_obj=""
pop = 0

levels={
0:"",
1:"",
2:"",
3:"",
4:"",
5:"",
6:"",
7:""}

obj_list=[0]
obj_index=1
print("--- %s seconds ---" % (time.time() - start_time))

ledger=""

for x in range(len(savegame)):
    prev_obj=cur_obj
        
    #maintenant que niveau est défini, on voit si c'est un objet
    if str(savegame[x]).strip().find("=")>=0:
        
        levels[cur_level]=do.get_left(str(savegame[x]).strip())
        
        param=do.get_left(str(savegame[x]).strip())
        value=do.get_right(str(savegame[x]).replace(".",",")).replace("\"","").strip()

#fini:classement des pops
        if levels[1] == "population":
            if param == "culture" :
                dico.popcul.update({levels[2]:value})
                pop +=1
            elif param == "religion" :
                dico.poprel.update({levels[2]:value})
                pop +=1
            elif param == "type" :
                dico.poptyp.update({levels[2]:value})
                pop +=1
            #if pop == 3 :
                # on a choppé les 3 paramètres, on peut imprimer
                
                #pop = 0
#en cours: préparation provinces
        if levels[0] == "provinces":
            if str(param).strip().find("buildings")>=0 and value == "{" and levels[1].isnumeric() == True :
                param="buildings"
                value=do.get_right(str(savegame[x+1]).strip().replace(" ","+"))
                value=eval(value)
            elif str(param).strip().find("date")>=0 or str(param).strip().find("change")>=0:
                value=do.get_right(str(savegame[x]).strip())
                
            elif str(param).strip().find("pop")>=0:
                param="ignore"
                dico.poploc.update({value:levels[1]})

            
            if do.is_info(levels[0])==1 and do.is_param(param)==1:
                do.province(levels[1],param,value)
                
        if levels[1] == "country_database":
            if do.is_excluded(param)==0 and do.is_excluded(levels[3])==0:
                ledger+='\n'+str(x)+";"+str(levels[2])+";"+str(levels[3])+";"+str(levels[4])+";"+str(param)+";"+str(value)
                
#to do: autres ("country","state") surtout
# diplomacy pour les guerres en cours (les passées?)



        
#la mécanique pour s'y retrouver dans les niveaux de save
    if str(savegame[x]).strip().find("{")>=0:
        cur_level+=1
                                
    if str(savegame[x]).strip().find("}")>=0:
        levels[cur_level]=""
        cur_level-=1
    #là on a déballé et tout rangé mais faut corriger des trucs avant de fermer le fichier
with open(defines.output+"output.csv", 'w', newline='') as output:
    provs = csv.writer(output,delimiter=';')
    with open(defines.output+"output2.csv", 'w', newline='') as output2:
        pops = csv.writer(output2,delimiter=';')
        pops.writerow(["pop_id","pop type","pop culture","pop religion","pop province id"])
        for i in dico.popcul:
            if dico.poploc.get(i)== None : dico.poploc.update({i:"spawning"})
            if i.isnumeric()==True :
                
                pops.writerow([i,dico.poptyp.get(i),dico.popcul.get(i),dico.poprel.get(i),dico.poploc.get(i)])
        provs.writerow(["prov_id","prov_name","prov_state","prov_owner","prov_controller","prov_previous_owner","prov_last_owner_change","prov_last_controller_change","prov_original_culture","prov_original_religion","prov_culture","prov_religion","prov_pop","prov_garrison","prov_civilization_value","prov_trade_goods","prov_num_of_roads","prov_num_foreign_culture","prov_province_rank","prov_buildings","prov_great_work"])
        for i in dico.prov_name:
            if i.isnumeric()==True :
            
                provs.writerow([i,dico.prov_name.get(i),dico.prov_state.get(i),dico.prov_owner.get(i),dico.prov_controller.get(i),dico.prov_previous_owner.get(i),dico.prov_last_owner_change.get(i),dico.prov_last_controller_change.get(i),dico.prov_original_culture.get(i),dico.prov_original_religion.get(i),dico.prov_culture.get(i),dico.prov_religion.get(i),dico.prov_pop.get(i),dico.prov_garrison.get(i),dico.prov_civilization_value.get(i),dico.prov_trade_goods.get(i),dico.prov_num_of_roads.get(i),dico.prov_num_foreign_culture.get(i),dico.prov_province_rank.get(i),dico.prov_buildings.get(i),dico.prov_great_work.get(i)])
        #à ce niveau, le fichier se ferme
with open(defines.output+"chercheur.txt", "w") as chercheur:
    chercheur.writelines(ledger)

#fin

old_name = defines.output+"output.csv"
new_name = defines.output+pays+"_"+year+"_provs.csv"

print (old_name)
print (new_name)
# Renaming the file
os.replace(old_name, new_name)

old_name = defines.output+"output2.csv"
new_name = defines.output+pays+"_"+year+"_pops.csv"

print (old_name)
print (new_name)
# Renaming the file
os.replace(old_name, new_name)

print("--- %s seconds ---" % (time.time() - start_time))
now = datetime.now()
dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
print(dt_string)
