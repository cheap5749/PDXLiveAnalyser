...
def extract_countries(source):
    ...
    for x in source["country"]["country_database"].keys():
        if int(x) < 100000 :
            ...
            for attr in country_attributes.keys():
                ...
                if "income" in attr:
                    attr_value = float(source["country"]["country_database"][x]["economy"][country_attributes[attr][1]][country_attributes[attr][2]])
    ...
    countries=pandas.DataFrame(currentobj, index=(x for x in source["country"]["country_database"].keys()))
    countries.columns = country_attributes.keys()
